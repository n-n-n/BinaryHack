...
int add_symbol(const char* name, int value, char* file_o)
{
    if (-1 == search_symbol_index(name)) {
        if (g_num < MAX_SYMBOL_NAME_NUM) {
            ...
            //      g_asymbolname[g_num].func = (void*)get_function(file_o, value);
            g_num++;
            return g_num - 1;
        }
    }
            ...
            else
            {
                ...
                if (-1 == index) {
                    ...
                    *p += (int)dlsym(RTLD_DEFAULT, name);
                    ...
                    int symbol_pos = g_asymbolname[g_num].value;
                    printf("relocate function (exp) %s\n", name);
                    g_asymbolname[g_num].func = (void*)get_function(file_o, symbol_pos);
...
//    LOG("[INFO] %s\n", name);
    ...
    if (!bfd_is_undefined_symclass(info.type)) {
      ...
      if (name[0] != '_') {
          ...
          LOG("Found %s :0x%08X\n", name, symbol_pos);
          add_symbol(name, symbol_pos, file_o);
      }
    }
  }
...
int main()
{
  ...
  //index = search_symbol_index("foo"); // can call a static function!
  //symbol_pos = g_asymbolname[index].func;
  ...
  func1("WORLD!");
/*
  index = search_symbol_index("hello_someone");
  symbol_pos = g_asymbolname[index].func;
  func1 = (void (*) (const char*))(symbol_pos);
  func1("WORLD!");
*/
  free(syms);
  bfd_close(abfd);
